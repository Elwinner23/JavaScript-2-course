...
const { detectCardType, validateCVVORCVCCode, validateExpirationDate, validateCreditCardNumber } = require("bb86cc72c43724a9");
...
const image = {
    visa: "https://github.com/KirillBorisovich/KirillBorisovich.github.io/blob/main/the%20best/app/img/visa.png?raw=true",
...
card_number.addEventListener("input", function(e) {
    ...
    if (inputText.length >= 14) {
        if (cardType == "visa") document.getElementById("foto").style.backgroundImage = `url('${image.visa}')`;
        else if (cardType == "mastercard") document.getElementById("foto").style.backgroundImage = `url('${image.mastercard}')`;
        else if (cardType == "discover") document.getElementById("foto").style.backgroundImage = `url('${image.discover}')`;
        else if (cardType == "american express") document.getElementById("foto").style.backgroundImage = `url('${image.amer_ex}')`;
    } else document.getElementById("foto").style.backgroundImage = "none";
    proverka();
});
...
},{"bb86cc72c43724a9":"ea7rk"}],"ea7rk":[function(require,module,exports) {
// Node Engine v16.14.2
function isCreditCard(card) {
    if (card.length >= 15 && card.length <= 19) return true;
    return false;
}
function isEncryptedToken(token) {
    if (token.length === 44) return true;
    return false;
}
function detectCardType(cardNumber) {
    ...
    if (dinersClubPattern.test(cardNumber)) return "diners club";
    else if (visaPattern.test(cardNumber)) return "visa";
    else if (mastercardPattern.test(cardNumber)) return "mastercard";
    else if (amexPattern.test(cardNumber)) return "american express";
    else if (discoverPattern.test(cardNumber)) return "discover";
    else return "invalid";
}
function validateCreditCardNumber(cardNumber) {
    ...
    if (cardNumber === "" || isNaN(cardNumber)) return false;
...
function validateCVVORCVCCode(securityCode) {
    ...
    if (securityCode === "" || isNaN(securityCode)) return false;
    // Check if the security code is 3 or 4 digits long
    var validLengths = [
        3,
        4
    ];
    if (!validLengths.includes(securityCode.length)) return false;
...
},{}]},["86oZd","6rimH"], "6rimH", "parcelRequire94c2")
